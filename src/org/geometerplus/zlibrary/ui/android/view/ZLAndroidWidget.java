...
public class ZLAndroidWidget extends View implements ZLViewWidget, View.OnLongClickListener {
	...
	public static boolean isRead2=true;
	...
	protected void onDraw(final Canvas canvas) {
		...
		if (context instanceof FBReader) {
			...
			System.err.println("A surprise: view's context is not an book");
		}
		super.onDraw(canvas);
...
//		final int h = getMainAreaHeight();
	...
	public void scrollManuallyTo(int x, int y,int i) {
		final ZLView view = ZLApplication.Instance().getCurrentView();
		final AnimationProvider animator = getAnimationProvider();
		if (view.canScroll(animator.getPageToScrollTo(x, y),true,i)) {
	...
	public void startAnimatedScrolling(ZLView.PageIndex pageIndex, int x, int y, ZLView.Direction direction, int speed) {
		final ZLView view = ZLApplication.Instance().getCurrentView();
		if (pageIndex == ZLView.PageIndex.current || !view.canScroll(pageIndex,false,0)) {
			return;
		}
		final AnimationProvider animator = getAnimationProvider();
	...
	public void startAnimatedScrolling(int x, int y, int speed) {
		if(isRead)
		{
			final ZLView view = ZLApplication.Instance().getCurrentView();
			final AnimationProvider animator = getAnimationProvider();
			if (!view.canScroll(animator.getPageToScrollTo(x, y),false,0)) {
				animator.terminate();
				return;
			}
				...
				if (myLongClickPerformed) {
					...
					if (myPendingPress) {
						if (isAMove) {
								...
								if (myPendingShortClickRunnable != null&&isRead) {
									removeCallbacks(myPendingShortClickRunnable);
									myPendingShortClickRunnable = null;
									
								}
								if (myPendingLongClickRunnable != null&&isRead) {
									removeCallbacks(myPendingLongClickRunnable);
